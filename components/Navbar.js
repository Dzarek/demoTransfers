  ...
  return (
    <>
      ...
      <Wrapper
        style={{
          borderBottom:
            router.pathname !== "/reservation" && "4px solid #b99e81",
        }}
      >
        <nav>
          {router.pathname !== "/" && (
            <Link href="/">
              <a className={isAdmin && "adminNav"}>
                <GiHomeGarage />
                <p className="aMobile">strona główna</p>
              </a>
            </Link>
          )}
          {router.pathname !== "/reservation" && !isAdmin && (
        ...
        <header>
          <h2 style={{ color: "#b6926c" }}>COMPANY LOGO</h2>
          {/* <img src={logoCompany} alt="" /> */}
          <h1>-</h1>
          <h2>transfery lotniskowe</h2>
        </header>
        {isAdmin ? (
          <div className="login loginAdmin">
            <span>
              <FaUserCircle /> {name}
            </span>
            <MdOutlineSettings
